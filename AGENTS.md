...
- `heimdall agent` is the stable automation entry point. Keep it read-only,
  single-document JSON, currently versioned as `heimdall.agent/v8`, and shell-safe by
  representing commands as argv arrays. Exit 0 means ready, 1 means not ready,
  and 2 remains clap usage failure. Additive v8 fields are allowed; renaming or

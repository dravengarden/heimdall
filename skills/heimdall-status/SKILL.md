...
No arguments. Reads the config path that the daemon is using
...
| `pod rules` | count of `podRouting.rules` entries |
| `default use` / `default observe` | the catchall decision |
| `relay listen` / `dns listen` | the eBPF redirect target + fake-IP DNS port |
| `flows in store` | count of rows in `flows.db` (use heimdall-flows for content) |
| `relay` | `ok` if the listen port accepts a TCP connection; `DOWN` otherwise |

## Failure interpretation

| Output | Diagnosis | Next step |
|---|---|---|
| `relay DOWN (port refused)` | daemon not running OR not bound | `systemctl status heimdall` then `journalctl -u heimdall -n 50` |
| `store ERROR: ...` | sqlite store unhealthy | check `state_dir` perms, disk space, schema version |

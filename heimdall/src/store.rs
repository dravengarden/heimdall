...
mod tests {
    ...
    async fn list_filters() {
        ...
        s.insert_flow_start(sample("conviva", "b.example.com")).await.unwrap();
        s.insert_flow_start(sample("conviva", "grafana.conviva.com")).await.unwrap();

        let all = s.list(ListQuery { limit: 100, ..Default::default() }).await.unwrap();
        assert_eq!(all.len(), 3);

        let conviva = s
            .list(ListQuery { limit: 100, connection: Some("conviva".into()), ..Default::default() })
            .await
            .unwrap();
        assert_eq!(conviva.len(), 2);

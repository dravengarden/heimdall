...
| macOS support | In development, not available | Darwin config/init, portable JSONL inspection, and an unavailable `heimdall.agent/v8` preflight now type-check without Linux dependencies; no backend emits macOS traffic evidence yet, and proxy execution, packaging, the signed companion, and native acceptance remain incomplete |

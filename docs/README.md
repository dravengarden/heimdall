...
- [design/macos-backend.md](design/macos-backend.md) — native-accepted Apple-silicon cooperative TCP source backend, unsigned companion/provider build skeleton, native package/signing/notarization boundary, and future signed transparent-provider contract
- [design/macos-control-protocol.md](design/macos-control-protocol.md) — internal Rust/Swift authenticated run-registration protocol and the native attribution evidence gate
...
- [runbook.md](runbook.md) — build, Darwin all-targets plus native Apple-silicon explicit/unsigned-companion/package gates, x86_64 NixOS/Ubuntu/Debian functional and performance VM gates, native-aarch64 acceptance, agent JSON contract, lifecycle, and troubleshooting

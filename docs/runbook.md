...
heimdall flows list --connection conviva
...
Web UI at `http://hawk.lan:9999/` (or `127.0.0.1:9999` from the host).
...
/etc/nixos/services/k0s/default.nix   the unit's source of truth
~/land/heimdall/                      source repo
~/land/heimdall/docs/                 you are here
```
...
|---|---|---|
| `use: conviva` + `observe: true` | Debug pod that needs to reach Conviva-internal hosts via the Mac SOCKS5, with plaintext capture | annotate pod: `heimdall.io/connection: conviva` |
| `use: conviva` + `observe: false` | Same routing, but suppress plaintext (e.g. running personal credentials through it) | both annotations |

...
the hawk node. Schema details live in `heimdall-config/src/lib.rs`;
this doc covers files-on-disk + the bootstrap workflow.

> Single source of truth: this file is at
> `~/land/heimdall/docs/config.md` and is symlinked into
> `/etc/nixos/docs/services/heimdall-config.md` and
> `~/conviva/docs/heimdall.md`. Edit here.
...
└── secrets/              ← 0700 root:root, NOT in git
    └── conviva.pw        ← 0400 root:root, no trailing newline
```
...
dispatch — **on hawk we use Nickel** because `lib.ncl` validates the
...
```
1. pod.annotations[routingKey]   ← annotation override (e.g. "system" / "conviva")
...
## Hawk's current rules (live config)

```
podRouting.rules:
  cluster-infra        ns ∈ {kube-system, local-path-storage}            → use=system
  conviva-workloads    label app.k8s.io/part-of=conviva  ∨  ns conviva*  → use=conviva, observe=true
...
`conviva` egress without touching this file:

```yaml
# values.yaml
podLabels:
  heimdall.io/routing: conviva
```
...
- Service module (NixOS): `/etc/nixos/services/heimdall/default.nix`
- Architecture / data flow: `/etc/nixos/docs/services/heimdall.md`
- Conviva networking context: `/home/draven/conviva/docs/networking.md`

...
- [`runbook.md`](runbook.md) — Build, Darwin config/evidence/log target type-check, resolver/userns and relay-CA preflight, x86_64 NixOS/Ubuntu/Debian lifecycle and fake DNS, TLS, real-eBPF and performance acceptance, host-guarded aarch64 acceptance, native/npm/PyPI/Cargo package gates, operation, and diagnosis
...
- [`design/macos-backend.md`](design/macos-backend.md) — In-development shared config/log scaffold, CLI-only explicit proxy, and optional signed `NETransparentProxyProvider` architecture, process attribution, lifecycle, capability boundaries, and native acceptance matrix
